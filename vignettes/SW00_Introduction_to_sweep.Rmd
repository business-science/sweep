...
2. fitted: the model values returned from the `ets()` function
3. forecast: the predicted values from the `forecast()` function

The `sw_sweep()` function contains an argument `fitted = TRUE` by default meaning that the "fitted" values are returned. We'll toggle this off in the next graph. The remaining columns are the forecast confidence intervals (typically 80 and 95, but this can be changed with `forecast(level = c(80, 95))`). These columns are setup to enable using the `geom_ribbon()` for visualization purposes. 


Let's see how to visualize with `ggplot2`. We can visualize using a combination of `geom_line()` and `geom_ribbon()`. The fitted values can be toggled off setting `fitted = FALSE`, which reduces the complexity of the plot. Note that because we are using a regular time index of the `yearmon` class, we need to add `scale_x_yearmon()`.
...
```{r}
sw_sweep(fcast_ets, fitted = FALSE, timekit_idx = TRUE) %>%
    ggplot(aes(x = index, y = price, color = key)) +
    geom_ribbon(aes(ymin = lo.95, ymax = hi.95), 
                fill = "#D5DBFF", color = NA, size = 0) +
    geom_ribbon(aes(ymin = lo.80, ymax = hi.80, fill = key), 
...
In this example, there is not much benefit to returning an irregular time series. However, when working with frequencies below monthly, the ability to return irregular index values becomes more apparent. This is a topic of discussion in the fourth vignette.
